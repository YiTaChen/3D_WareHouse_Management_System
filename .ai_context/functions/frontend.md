...
- MoveTable renders the original plateTable collider invisibly at the full mission offset, but splits the replacement visual into `ForkFixedAssembly` and `ForkExtendingTines`. The carrier and two outer guides ignore local Z extension; the two inner tines remain anchored in those guides while visually lengthening to the cargo target.
