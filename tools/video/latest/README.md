...
For future reproduction, install Node.js, Python 3 and Chrome, then run `npm install` and `npm run produce` in this directory. The recorder installs the application dependencies, creates a disposable SQLite database, records the real WebGL canvas and emits `.video-production/warehouse-latest-demo.mp4`. Set `CHROME_PATH` or `FFMPEG` if needed. The output is 60 seconds, 1920×1080, 30 fps H.264 with no audio. Real-time physics timing can vary by hardware. Servers stop after recording.

The first two recorded seconds are trimmed so the README player opens on the actual warehouse instead of a black fade-in.
